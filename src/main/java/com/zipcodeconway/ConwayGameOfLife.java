...
public class ConwayGameOfLife {
    ...
    private int[][] createRandomStart(Integer dimension) {
        ...
        for (int row = 0; row < myArray.length; row++) {
            for (int col = 0; col < myArray.length; col++){
                myArray[row][col] = (int)Math.round(Math.random());
            }
        }
        return myArray;
    }
    ...
    public int[][] simulate(Integer maxGenerations) {
        ...
        while (generations <= maxGenerations) {
            ...
            for (int row = 0; row < currentGen.length; row++) {
                for (int col = 0; col < currentGen.length; col++) { //updating currentGen to nextGen
                    nextGen[row][col] = isAlive(row, col, currentGen);
                }
            }
            copyAndZeroOut(nextGen, currentGen);
            this.displayWindow.sleep(125);
            generations++;  //killing the while loop
        }
        return currentGen;
    }
	...
	*/
    private int isAlive(int row, int col, int[][] world) {
        ...
        // these are boundaries we loop around
